...
export function SecretarySidebar() {
      ...
      >
        ...
        <Icon
          className={cn(
            ...
            active ? "text-teal-700" : "text-white/90 group-hover:text-white"
          )}
        />
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-6 py-6 border-b border-white/10">
        ...
        <div className="mt-4 px-2">
          <div className="flex items-center gap-2 px-3 py-2 bg-teal-600/30 rounded-lg border border-teal-400/30">
      ...
      <div className="px-4 py-4 border-t border-white/10 bg-white/5">
        <div className="flex items-center gap-3 mb-3">
          <Avatar className="h-10 w-10 border-2 border-teal-300/30">
            <AvatarImage src={avatarUrl || undefined} alt={user.username} />
            <AvatarFallback className="bg-teal-400/20 text-teal-200 border-teal-300/30 font-semibold">
              {avatarUrl ? (
      ...
      {isMobileMenuOpen && (
        <>
          <div
            ...
            onClick={() => setIsMobileMenuOpen(false)}
          />
          <div className="lg:hidden fixed inset-y-0 left-0 z-50 w-[280px] bg-teal-700 shadow-2xl transform transition-transform duration-300 ease-in-out">
  ...
  return (
    <>
      <MobileSidebar />
      <aside className="hidden lg:flex flex-col fixed left-0 top-0 h-full w-[240px] bg-teal-700 z-30">
        <SidebarContent />
      </aside>
    </>
  );
